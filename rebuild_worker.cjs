...
fileContent = fileContent.replace(
  /<div key={issue\.id} className="worker-card">/g,
  "<div key={issue.id} className=\"worker-card\" style={{ borderLeft: `4px solid ${!hasPhoto ? 'var(--color-orange)' : 'var(--color-green)'}` }}>"
);

// 6. Production Tab
fileContent = fileContent.replace(
  /<div className="worker-card" style={{ padding: '20px' }}>/g,
  "<div className=\"worker-card\" style={{ padding: '20px', borderLeft: '4px solid var(--primary)' }}>"
);
// Fix the second card (which had padding:20px but needs teal border)
fileContent = fileContent.replace(
  /<label style={{ fontSize: '12px', fontWeight: 700, color: 'var\(--text-primary\)', marginBottom: '12px', display: 'block' }}>/g,
...
// Wait, the above replace will duplicate the worker-card if not careful.
// Instead, let's just use string replacement on the exact block.

// 7. Earnings Tab (The map function)
const oldEarningsBlock = `                    workerProduction.map(p => (
...
const newEarningsBlock = `                    workerProduction.map(p => (
...
fileContent = fileContent.replace(oldEarningsBlock, newEarningsBlock);

// 8. Notifications Tab -> Profile Tab
const oldNotificationsTab = `{activeTab === 'notifications' && (
...
const newProfileTab = `{activeTab === 'profile' && (
...
fileContent = fileContent.replace(oldNotificationsTab, newProfileTab);

// 9. Bottom Nav Notifications -> Profile
const oldNav = `<div className={\`bottom-nav-item \${activeTab === 'notifications' ? 'active' : ''}\`} onClick={() => setActiveTab('notifications')}>
              <Bell size={22} />
              <span>Profile</span>
...
const newNav = `<div className={\`bottom-nav-item \${activeTab === 'profile' ? 'active' : ''}\`} onClick={() => setActiveTab('profile')}>
              <User size={22} />
              <span>Profile</span>
            </div>`;

fileContent = fileContent.replace(oldNav, newNav);
...
console.log('Successfully rebuilt worker-app App.tsx');
