...
// 3. Update the login UI Wrapper (Make it use worker-app-container)
content = content.replace(
...
// 4. Update the main app Wrapper (After login)
content = content.replace(
...
// 5. Replace bottom nav (which also contains the unclickable <a> tags that caused the issue)
...
// 6. Clean up trailing closing divs since we removed device-container, phone-frame, phone-screen from the two wrappers!
// For login:
content = content.replace(
